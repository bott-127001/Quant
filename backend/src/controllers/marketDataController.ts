...
export const getDashboardData = async (req: Request, res: Response) => {
    try {
        ...
        // 4. Get Upstox Auth Status
        const auth = await Auth.findOne().sort({ createdAt: -1 });
        let sessionStatus: 'VALID' | 'EXPIRED' | 'MISSING' = 'MISSING';
        if (auth) {
            const twentyFourHours = 24 * 60 * 60 * 1000;
            const elapsed = Date.now() - auth.createdAt.getTime();
            sessionStatus = elapsed < twentyFourHours ? 'VALID' : 'EXPIRED';
        }
