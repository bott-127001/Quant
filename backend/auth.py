...
async def login(user: str = Query(...)):
    ...
    """
    print(f"\n{'='*70}")
    print(f"INFO: LOGIN REQUEST RECEIVED - User clicked login button for: {user}")
    print(f"{'='*70}")
    ...
    # import traceback
    # print(f"\n{'='*70}")
    # print(f"INFO: OAuth Login Request for user: {user}")
    # print(f"{'='*70}")
    # print(f"WARNING: This endpoint was called!")
    ...
    # print(f"State: {user}")
    # print(f"\nIMPORTANT: The Redirect URI above MUST match EXACTLY in Upstox app settings!")
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if state == "samarth":
                ...
                print(f"SUCCESS: Also stored tokens for prajwal (samarth's login feeds both users)")
            
            # Verify tokens were stored correctly
            stored_tokens = await get_user_tokens(state)
            if not stored_tokens or not stored_tokens.get("access_token"):
                print(f"ERROR: ERROR: Tokens were not stored correctly for {state}")
            ...
            print(f"SUCCESS: Authentication successful for user: {state}")
            print(f"SUCCESS: Access token stored: {access_token[:20]}...")
            print(f"INFO: Token expires at 3 AM IST (in {expires_in_hours:.1f} hours)")
            print(f"SUCCESS: Token storage verified in database")
            print(f"SUCCESS: Polling enabled - data fetching will start immediately")
...
async def logout():
    ...
    if current_user:
        ...
        disable_polling()
        print(f"SUCCESS: Logged out {current_user} - tokens cleared and polling stopped")
...
async def refresh_access_token(username: str) -> Optional[str]:
    ...
    """
    print(f"WARNING: Token refresh attempted for {username}, but Upstox doesn't support refresh tokens.")
    print(f"WARNING: Tokens expire at 3 AM IST daily. User needs to re-login.")
    return None
...
async def frontend_login(request: Request):
    ...
    try:
        ...
        print(f"INFO: Frontend login attempt - Username: {username}")
        ...
        print(f"SUCCESS: Frontend login successful for {username}")
        ...
        print(f"ERROR: Frontend login error: {str(e)}")
    ...
    try:
        ...
        if auth_header and auth_header.startswith("Bearer "):
            ...
            if session:
                ...
                print(f"SUCCESS: Frontend logout successful for {username}")
        ...
        print(f"ERROR: Frontend logout error: {str(e)}")
...
async def frontend_check(request: Request):
    ...
    try:
        ...
        print(f"ERROR: Frontend check error: {str(e)}")
        return JSONResponse(content={"authenticated": False})
    ...
    try:
        ...
        if updated_at:
            ...
            try:
                ...
                print(f"WARNING: Error parsing updated_at: {e}")
                logged_in_today = False
        ...
        print(f"ERROR: Error checking Upstox login status: {str(e)}")
        import traceback
        traceback.print_exc()
...
async def trigger_upstox_login(request: Request):
    ...
    try:
        ...
        print(f"INFO: Manual Upstox login triggered for {username}")
        ...
        if success:
            # Enable polling after successful login
            enable_polling()
            print(f"SUCCESS: Manual Upstox login successful for {username}")
            return JSONResponse(content={
                "success": True,
                "message": f"Upstox login successful for {username}"
            })
        else:
            print(f"ERROR: Manual Upstox login failed for {username}")
            return JSONResponse(content={
                "success": False,
        ...
        print(f"ERROR: Error triggering Upstox login: {str(e)}")
        import traceback
        traceback.print_exc()
        raise HTTPException(status_code=500, detail=f"Error triggering login: {str(e)}")

@auth_router.post("/trigger-selection")
async def trigger_elite_selection(request: Request):
    """
    Manually trigger Elite 10 selection engine.
    """
    try:
        auth_header = request.headers.get("Authorization")
        if not auth_header or not auth_header.startswith("Bearer "):
        ...
        print(f"INFO: Manual Elite 10 selection triggered by {username}")
        
        from elite_scheduler import run_daily_selection
        asyncio.create_task(run_daily_selection(username))
        
        return JSONResponse(content={"success": True, "message": "Elite 10 selection process started. This may take up to 60 seconds."})
        
    except HTTPException:
        raise
    except Exception as e:
        print(f"ERROR: Error triggering Elite selection: {str(e)}")
        import traceback
        traceback.print_exc()
        raise HTTPException(status_code=500, detail=f"Error starting selection: {str(e)}")
