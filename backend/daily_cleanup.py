...
async def null_out_tokens():
    ...
    try:
        ...
        print(f"SUCCESS: Nulled out tokens for {result.modified_count} users")
...
async def daily_cleanup_task():
    ...
    try:
        ...
        print(f"\nERROR: Error during daily cleanup: {str(e)}")
        import traceback
        traceback.print_exc()
        print(f"{'='*70}\n")
...
async def token_cleanup_scheduler():
    ...
    """
    print("INFO: Token cleanup scheduler started (runs at 3:00 AM IST)")
    ...
    while True:
        try:
            ...
            print(f"INFO: Next token cleanup scheduled for: {next_cleanup.strftime('%Y-%m-%d %H:%M:%S IST')}")
            ...
            print(f"\n{'='*70}")
            print(f"INFO: Starting token cleanup at {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')}")
            print(f"{'='*70}")
            await null_out_tokens()
            print(f"SUCCESS: Token cleanup completed")
            print(f"{'='*70}\n")
            
        except Exception as e:
            print(f"ERROR: Error in token cleanup scheduler: {str(e)}")
            import traceback
            traceback.print_exc()
