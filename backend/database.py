...
if MONGO_URI and ("mongodb.net" in MONGO_URI or "mongodb+srv://" in MONGO_URI):
    ...
    if MONGO_URI:
        ...
        print("WARNING: MONGO_URI not set in .env file. Attempting to connect to local MongoDB...")
...
historical_daily_prices_collection = db.get_collection("historical_daily_prices")
elite_10_daily_collection = db.get_collection("elite_10_daily")
...
async def init_db():
    ...
    try:
        ...
        if not MONGO_URI:
            print("WARNING: Tip: Set MONGO_URI in your .env file with your MongoDB connection string")
    ...
    await historical_daily_prices_collection.create_index([("symbol", 1), ("timestamp", -1)])
    await elite_10_daily_collection.create_index("date", unique=True)
...
async def update_user_settings(username: str, settings: Dict) -> Optional[Dict]:
    ...
    regular_fields = {
        ...
        "index_gravity_threshold": settings.get("index_gravity_threshold"),
...
async def init_frontend_users():
    ...
    for user in ["samarth", "prajwal"]:
        ...
        if not password:
            print(f"WARNING: Warning: {password_env_key} not set in .env. Frontend user '{user}' will not be created.")
        ...
        if result.upserted_id:
            print(f"SUCCESS: Created frontend user: {user}")
        # If user already exists, silently skip (no error)

async def store_historical_daily_data(symbol: str, daily_data: List[Dict]):
    """
    Store 30 days of 1-day timeframe data for a stock.
    daily_data: List of dicts with {timestamp: datetime, open: float, high: float, low: float, close: float, volume: int}
    """
    if not daily_data:
        return
    
    # Use bulk write to update/insert multiple records
    ...
    if ops:
        await historical_daily_prices_collection.bulk_write(ops)

async def get_historical_daily_data(symbol: str, days: int = 30) -> List[Dict]:
    """Fetch stored daily data for a stock."""
    cursor = historical_daily_prices_collection.find({"symbol": symbol}).sort("timestamp", -1).limit(days)
    data = await cursor.to_list(length=days)
    return data

async def get_latest_historical_date(symbol: str) -> Optional[datetime]:
    """Get the most recent timestamp of stored data for a stock."""
    doc = await historical_daily_prices_collection.find_one(
        {"symbol": symbol},
        sort=[("timestamp", -1)]
    )
...
async def store_elite_10(date_str: str, symbols: List[str], metadata: Dict = None):
    """
    Store the daily Elite 10 stocks.
    date_str: "YYYY-MM-DD"
    """
    await elite_10_daily_collection.update_one(
        {"date": date_str},
        {
            "$set": {
                "date": date_str,
...
async def get_elite_10(date_str: str) -> Optional[List[str]]:
    """Get the Elite 10 for a specific date."""
    doc = await elite_10_daily_collection.find_one({"date": date_str})
    if doc:
        return doc.get("symbols", [])
    return None

async def prune_historical_data(days_to_keep: int = 40):
    """Remove data older than X days to keep DB clean."""
    from datetime import timedelta
    cutoff = datetime.utcnow() - timedelta(days=days_to_keep)
    await historical_daily_prices_collection.delete_many({"timestamp": {"$lt": cutoff}})
