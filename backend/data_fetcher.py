...
# Track which users we've notified about stale tokens for today
_stale_token_notified = set()
...
_adr_cache: Dict[str, float] = {} # {symbol: avg_adr} Cached ADR values to avoid calculating every 5s
...
async def fetch_previous_day_ohlc(username: str, instrument_key: str, target_date: str) -> Optional[Dict]:
    ...
    if updated_at:
        try:
            ...
            if token_date_str != today_str:
                print(f"INFO: Token for {username} is from {token_date_str}, not today. Cannot fetch previous-day OHLC.")
                return None
        except Exception as e:
            print(f"WARNING: Error checking token date for previous-day OHLC: {e}")
            return None
    ...
    if not access_token or token_expires_at <= (time.time() + 60):
        print(f"ERROR: Token not valid for {username} when fetching previous-day OHLC.")
        return None
...
async def fetch_option_chain(username: str) -> Optional[Dict]:
    ...
    if updated_at:
        try:
            ...
            token_date_str = updated_ist.strftime("%Y-%m-%d")
            if token_date_str != today_str:
                print(f"INFO: Token for {username} is from {token_date_str}, not today. Waiting for new login...")
                return None
        except Exception as e:
            print(f"WARNING: Error checking token date: {e}")
            return None
    ...
    if not access_token or token_expires_at <= (time.time() + 60):
        if token_expires_at <= time.time():
            print(f"ERROR: Token expired for user: {username} (expired at 3 AM IST). User needs to re-login.")
        else:
            print(f"WARNING: Token expiring soon for user: {username} (expires at 3 AM IST). User should re-login.")
    ...
    # print(f"INFO: Fetching option chain - Instrument: {instrument_key}, Expiry: {expiry_date}")
...
def normalize_option_chain(upstox_data: Dict) -> Dict:
    ...
    try:
        # print(f"INFO: Normalizing option chain data...")
...
async def get_daily_baseline(username: str, date_str: str) -> Optional[Dict]:
    ...
    if baseline_doc:
        print(f"INFO: Found existing baseline in DB for {username} on {date_str}")
...
async def save_daily_baseline(username: str, date_str: str, baseline_data: Dict):
    ...
    print(f"INFO: Saving baseline to DB for {username} on {date_str}")
...
async def clear_daily_baseline(username: str, date_str: str):
    ...
    if result.deleted_count > 0:
        print(f"INFO: Cleared baseline from DB for {username} on {date_str}")
...
def update_price_history(current_price: float, current_time: datetime):
    ...
    if market_open_time is None or today_market_open.date() != market_open_time.date():
        ...
        if open_price_from_candle is not None:
            ...
            print(f"INFO: New trading day detected. Using accurate open price from 9:15 AM candle: {open_price}")
        else:
            open_price = current_price
            print(f"INFO: New trading day detected. Open price (from spot price): {open_price}")
...
async def fetch_current_day_open_candle(username: str, instrument_key: str) -> Optional[float]:
    ...
    if current_time_ist < earliest_fetch_time:
        print(f"INFO: Too early to fetch 9:15 AM candle ({current_time_ist.strftime('%H:%M')}). Waiting until after 9:16 AM IST...")
        return None
    ...
    if updated_at:
        try:
            ...
            token_date_str = updated_ist.strftime("%Y-%m-%d")
            if token_date_str != today_str:
                print(f"INFO: Token for {username} is from {token_date_str}, not today. Cannot fetch current-day open candle.")
                return None
        except Exception as e:
            print(f"WARNING: Error checking token date for current-day open candle: {e}")
            return None
    ...
    token_expires_at = tokens.get("token_expires_at", 0)
    if not access_token or token_expires_at <= (time.time() + 60):
        print(f"ERROR: Token not valid for {username} when fetching current-day open candle.")
        return None
    ...
    print(f"INFO: Fetching intraday candle from: {url}")
    ...
    try:
        ...
        candles = data.get("data", {}).get("candles", [])
        if not candles:
            print(f"WARNING: No candles returned for {instrument_key} on {today_str}. API Response: {data}")
            return None
        ...
        if open_price_val is None:
            ...
            print(f"WARNING: Exact 9:15 AM candle not found. Earliest available is: {timestamp_str}")
            ...
            if today_str in timestamp_str and ("T09:1" in timestamp_str or "T09:2" in timestamp_str):
                 ...
                 print(f"WARNING: Using earliest available candle ({timestamp_str}) as open price.")
            else:
                 print(f"ERROR: Earliest candle {timestamp_str} is too late or wrong date. Cannot determine open price.")
                 return None

        print(f"SUCCESS: Current day open candle fetched for {username} - open_price={open_price_val} on {today_str}")
...
async def fetch_and_store_previous_day_data(username: str) -> Optional[Dict]:
    ...
    for days_back in range(max_days_back):
        ...
        if ohlc:
            ...
            if not updated:
                print(f"ERROR: Failed to update settings with previous-day data for {username}")
                return None

            print(
                f"SUCCESS: Previous-day data stored for {username} - "
    ...
    print(f"ERROR: Failed to fetch previous-day OHLC for {username} after trying {max_days_back} days back (may be a market holiday)")
    return None
...
async def polling_worker():
    ...
    while polling_active:
        ...
        if not (start_time <= current_time <= end_time) or now_ist.weekday() >= 5:
            # Outside market hours or weekend
            if current_user:
                print(f"INFO: Market closed ({current_time.strftime('%H:%M')}). Sending offline data to frontend.")
                # We still want the frontend to show the generated Elite 10 even when market is closed
                from database import get_elite_10
                today_date_str = now_ist.strftime("%Y-%m-%d")
                elite_symbols = await get_elite_10(today_date_str)
                ...
                latest_data = {
                    ...
                    "index_status": {"gap": 0, "ltp": 0},
                    "option_count": 0,
                    "options": [],
                    "message": "Market Closed. Displaying today's Elite 10 stocks."
                }
                
                # Broadcast the offline state
                if manager:
                    try:
                        await manager.broadcast(latest_data)
                    except Exception as e:
                        print(f"WS broadcast error (offline mode): {e}")
        ...
        for user in ["samarth", "prajwal"]:
            ...
            if updated_at:
                try:
                    ...
                    if token_date_str == today_str:
                        found_user = user
                        # Clear notification flag if they login today
                        if f"{user}_{today_str}" in _stale_token_notified:
                            _stale_token_notified.remove(f"{user}_{today_str}")
                        break
                    else:
                        # Token is from previous day - ignore it, wait for new login
                        # Only print this once per day to avoid spamming the logs
                        notify_key = f"{user}_{today_str}"
                        if notify_key not in _stale_token_notified:
                            print(f"INFO: Token for {user} is from {token_date_str}, waiting for today's login...")
                            _stale_token_notified.add(notify_key)
                except Exception as e:
                    print(f"WARNING: Error checking token date for {user}: {e}")
        ...
        if not found_user:
            ...
            if current_user:
                print(f"WARNING: No authenticated user available with today's tokens. Waiting for automated login at 9:15 AM...")
                current_user = None
                latest_data = None
        ...
        if not should_poll:
            if found_user:
                print(f"INFO: Found valid token for {found_user}, auto-enabling polling.")
                should_poll = True
            else:
        ...
        if found_user != current_user:
            ...
            print(f"SUCCESS: Authenticated user found: {current_user}")
        ...
        try:
            ...
            try:
                ...
                print(f"WARNING: API call timeout for {current_user}")
            ...
            if upstox_data:
                ...
                if not normalized_data:
                    print("WARNING: Failed to normalize option chain data")
                    await asyncio.sleep(5)
                    continue
                
                if not normalized_data.get("options"):
                    print(f"WARNING: No options found in normalized data. Keys: {list(normalized_data.keys())}")
                    await asyncio.sleep(5)
                    continue
                ...
                try:
                    ...
                    if is_baseline_invalid and aggregated:
                        ...
                        print("INFO: Baseline greeks captured for the day.")
                        today_str = datetime.now(timezone.utc).strftime("%Y-%m-%d")
                    ...
                    try:
                        ...
                        if last_fetched_date != today_str:
                            ...
                            if ohlc:
                                ...
                                print(
                                    f"WARNING: Auto-fetch of previous-day data failed for {current_user}. "
                                    f"Manual fetch can be triggered from Dashboard."
                                )
                    except Exception as e:
                        print(f"WARNING: Error in auto-fetching previous-day data for {current_user}: {e}")
                        ...
                        if last_open_candle_date != today_str:
                            ...
                            if candle_open_price is not None:
                                ...
                                if open_price != candle_open_price:
                                    ...
                                    print(f"INFO: Updated day's open price to accurate candle open: {open_price}")
                                print(f"INFO: Using accurate market open price from 9:15 AM candle: {candle_open_price}")
                            else:
                                print(f"WARNING: Could not fetch current-day open candle for {current_user}. Will use first spot price.")
                    except Exception as e:
                        print(f"WARNING: Error in auto-fetching current-day open candle for {current_user}: {e}")
                    ...
                    if open_price is None or market_open_time is None:
                        print(f"WARNING: Skipping metrics: open_price or market_open_time is not yet available. This can happen after a state reset.")
                    ...
                    # Determine current Elite 10 stocks for the frontend
                    current_date = (datetime.now(timezone.utc) + timedelta(hours=5, minutes=30)).strftime("%Y-%m-%d")
                    elite_symbols = await get_elite_10(current_date)
                    elite_stocks_data = []
                    ...
                    if elite_symbols:
                        try:
                            ...
                            # Using the active credentials
                            req_headers = {"Accept": "application/json", "Authorization": f"Bearer {tokens['access_token']}"}
                            ...
                            if resp.status_code == 200:
                                ...
                                for raw_sym, sym_data in resp_data.items():
                                    actual_key = sym_data.get("instrument_token")
                                    if not actual_key:
                                        actual_key = raw_sym.replace(":", "|")
                                    
                                    # Fetch/Calculate ADR using cache
                                    if _adr_cache_date != current_date:
                                        _adr_cache.clear()
                                        _adr_cache_date = current_date
                                    ...
                                    elite_stocks_data.append({
                                        ...
                                        "rel_vol": 1.5, # TODO calculate or fetch avg_915_vol
                                        "avg_adr_5d": _adr_cache[actual_key] if _adr_cache[actual_key] > 0 else 10.0,
                                        "irs_1100": 0.0
                                    })
                        except Exception as e:
                            print(f"WARNING: Error fetching Elite 10 market quotes: {e}")

                    # Elite 10 Signal Detection
                    elite_signals = await detect_elite_signals(
                        ...
                        index_data={
                            "ltp": current_price, 
                            "open": open_price_from_candle if open_price_from_candle is not None else open_price,
                            "prev_close": settings.get("prev_day_close", 0)
                        },
                    ...
                    latest_data = {
                        ...
                        "elite_10": elite_symbols or [],
                        "index_status": {
                            "gap": (open_price - settings.get("prev_day_close", 0)) / settings.get("prev_day_close", 1) * 100 if settings.get("prev_day_close") else 0,
                            "ltp": current_price
                        },
            ...
            if _last_successful_poll:
                ...
                if time_since_success > 30 and not _stall_warning_sent:
                    print(f"WARNING: STALL DETECTED: No successful poll in {time_since_success:.1f} seconds")
...
def start_polling():
    ...
    if _polling_task is not None and not _polling_task.done():
        print("WARNING: Polling worker already running, skipping duplicate start")
        return
    
    try:
        _polling_task = asyncio.create_task(polling_worker())
    except RuntimeError:
        print("ERROR: Cannot start polling without a running event loop")
...
def enable_polling():
    ...
    should_poll = True
    print("SUCCESS: Polling enabled - will start fetching data with today's fresh tokens")
    
    # Auto-start the polling worker loop if it isn't running
    start_polling()
...
def disable_polling():
    ...
    print("STOPPED: Polling disabled - will stop fetching data")
...
def reset_baseline_greeks():
    ...
    print("INFO: In-memory baseline greeks and price history have been reset. A new baseline will be captured on the next poll.")
